...
; #INDEX# =======================================================================================================================
_SoundOpen("sounds_Encripted\SOUNDSDATA.DAT\bound.ogg.enc")
; Title .........: Sound
; AutoIt Version : 3.3.14.5
; Language ......: English
; Description ...: Functions that assist with Sound management.
; Author(s) .....: RazerM, Melba23, Simucal, PsaltyDS, Mateo Cedillo.
; Dll ...........: winmm.dll
; ===============================================================================================================================
...
func _soundEncrypt($Extension)
$freq = 440
$listFiles =  _GetFilesFolder_Rekursiv("Sounds\soundsdata.dat", $Extension, 0, 0)
For $i = 1 to $listFiles[0]
beep($freq, 50)
$freq = $freq +9
$opensound2 = FileOpen($listfiles[$i], $FO_READ)
$opennedsoun = FileRead($opensound2)
$encoded = _Base64Encode($opennedsoun)
if @error then
msgbox(0, "Error", @error)
EndIf
$filenuevo = $listfiles[$i] &".enc"
FileWrite($filenuevo, $encoded)
FileClose($opennedsoun)
Sleep(200)
Next
...
$opensound2 = FileOpen($listfiles[$i], $FO_READ)
$opennedsoun = FileRead($opensound2)
$encoded = _Base64Decode($opennedsoun)
if @error then
msgbox(0, "Error", @error)
EndIf
$filenuevo = $listfiles[$i] &"." &$extension
FileWrite($filenuevo, $encoded)
FileClose($opennedsoun)
Sleep(200)
Next
...
Func _SoundOpen($sFilePath)
	...
	If $bTryNextMethod Then
		...
		If $sExt = ".mp3" Then
			Local $hFileenc = FileOpen(FileGetShortName($sSndDirName & $sSndFileName), $FO_READ)
			Local $hFilereadenc = FileRead($hFileenc)
			Local $hFile = _Base64Decode($hFilereadenc)
